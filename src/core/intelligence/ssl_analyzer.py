...
class SSLAnalyzer:
    """
    SSL/TLS Analyzer v1.0.
    Extracts full certificate details, TLS version, cipher suite, and crt.sh logs.
    ...
    def analyze(self) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.error(f"[-] SSL Analysis failed for {self.domain}: {e}")
    ...
    def _get_crt_sh(self) -> List[Dict]:
        try:
            ...
            if resp.status_code == 200:
                return resp.json()[:20] # Top 20 latest
        except Exception:
            pass
        return []

    def run(self):
        self.logger.info(f"[*] Starting Deep SSL/TLS Analysis for {self.domain}...")
        return self.analyze()
